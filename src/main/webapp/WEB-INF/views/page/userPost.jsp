			...
			<div id="content">
				<div style="text-align: center; padding-top: 150px;">
					...
					<ul class="nav nav-tabs">
						<li class="nav-item">
							<a class="nav-link active" href="/page/userPost?user_idx=${pagewowner.user_idx}">포스트</a>
						</li>
						<li class="nav-item">
				...
				<div style="text-align: center; padding-top: 100px; padding-bottom: 20px;">
					<c:if test="${log_in.user_idx==pagewowner.user_idx}">
						<h1 style="font-size: 23pt">내가 쓴 글</h1>
					</c:if>
					<c:if test="${log_in.user_idx!=pagewowner.user_idx}">
						<h1 style="font-size: 23pt">${pagewowner.nickname}님의 포스트</h1>
					</c:if>					
				</div>
				...
				<div class="container">
					<table class="table table-hover">
						...
						<tbody>
							<c:forEach var="post" items="${user_post}" varStatus="status">
								...
								<tr>
								<c:set var="num" value="${fn:length(user_post)-status.index}" />
									<td><c:out value="${num}" /></td>
									<td colspan=2><a href="post/${post.post_id}" style="text-decoration: none; font-weight: bold; color: grey">
										${post.title}</a>
									</td>
